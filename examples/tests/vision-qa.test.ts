...
// ---------------------------------------------------------------------------
// runVisionQA — provider wiring (mock createProvider)
// ---------------------------------------------------------------------------
...
describe("runVisionQA", () => {
  it("calls provider with correct parameters", async () => {
    ...
    try {
      const findings = [
        {
          ...
          suggestion: "Use a type scale",
        },
      ];
      const mockChatFn = jest.fn().mockResolvedValue({
        textBlocks: [JSON.stringify({ findings, summary: "Minor consistency issues." })],
        toolCalls: [],
        stopReason: "end_turn",
      });
      const mockProvider: LLMProvider = { name: "mock", chat: mockChatFn };

      // Import here so we can pass a pre-built provider via options
      const { runVisionQA } = await import("../skills/vision-qa-agent");

      const report = await runVisionQA(tmpImage, {
        context: "dashboard",
      ...
      const req: ChatRequest = mockChatFn.mock.calls[0][0];
      expect(req.model).toBe("gemini-2.0-flash");
      expect(req.messages[0].role).toBe("user");

      expect(report.findings).toHaveLength(1);
      expect(report.summary).toBe("Minor consistency issues.");
      expect(typeof report.analyzedAt).toBe("string");
    } finally {
      fs.unlinkSync(tmpImage);
    }
